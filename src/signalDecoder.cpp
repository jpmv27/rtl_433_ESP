...
#if defined(RTL433_ALL_DECODER_ANALYZE) || defined(RTL433_DECODER_ANALYZE)
#  define rtl_433_Decoder_Stack 60000
#elif defined(RTL433_DECODER_VERBOSE) || defined(RTL433_ALL_DECODER_VERBOSE)
...
#ifndef RTL433_DECODER_QUEUE_SIZE
#  define RTL433_DECODER_QUEUE_SIZE 5
#endif
...
// logprintfLn(LOG_INFO, "Location of cfg->devices: %p", (void *)&cfg->devices);
    ...
    cfg->verbosity = RTL433_ALL_DECODER_VERBOSE + 5; // 0=normal, 1=verbose, 2=verbose decoders,
    ...
    for (int i = 0; i < cfg->num_r_devices; i++) {
      ...
      char* arg = NULL;
      char verbose[4] = "vvv";
#ifdef RTL433_DECODER_VERBOSE
      if (RTL433_DECODER_VERBOSE == i) {
        arg = verbose;
      }
#endif
...
#endif
    rtl_433_Queue = xQueueCreate(RTL433_DECODER_QUEUE_SIZE, sizeof(pulse_data_t*));
...
#  endif
    ...
    if (events == 0) {
      ...
      logprintf(LOG_INFO, "RAW (%lu): ", rtl_pulses->signalDuration);
#  ifndef RTL433_RAW_SIGNAL_DEBUG
      for (int i = 0; i < rtl_pulses->num_pulses; i++) {
        alogprintf(LOG_INFO, "+%d", rtl_pulses->pulse[i]);
        alogprintf(LOG_INFO, "-%d", rtl_pulses->gap[i]);
#    ifdef RTL433_SIGNAL_RSSI
...
void processSignal(pulse_data_t* rtl_pulses) {
  ...
  if (xQueueSend(rtl_433_Queue, &rtl_pulses, 0) != pdTRUE) {
    rtl_433_ESP::decoderQueueOverflows++;
